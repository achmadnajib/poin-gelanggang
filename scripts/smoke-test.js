...
(async()=>{
  ...
  x=await request(`/api/matches/${m.id}/control`,{method:'POST',cookie,body:{action:'next'}});assert.equal(x.data.round,2);
  ...
  assert.equal(duplicate.startedAt,null);assert.equal(duplicate.winner,null);assert.equal(duplicate.victoryReason,'');
  ...
  console.log(JSON.stringify({ok:true,base,matchCode:m.code,checks:43}));
